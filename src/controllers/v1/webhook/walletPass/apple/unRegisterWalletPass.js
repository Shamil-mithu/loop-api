...
const { CustomerBalance, Brand, Order, AppleWalletPass } = require("@src/models");
...
const CONTROLLER = [
    ...
    async function unRegisterWalletPassV1Controller(req, res) {
        try {
            ...
            findPass.is_actve = false;
            findPass.is_deleted = true;
            findPass.deleted_at = new Date();
            await findPass.save();
            //ask CTO if we need to delete the pass from s3 bucket or not
            
            console.log(`UN REGISTER API HAS BEEN HIT for serial number ${serialNumber}`)
