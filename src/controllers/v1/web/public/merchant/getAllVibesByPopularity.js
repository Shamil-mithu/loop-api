...
const CONTROLLER = [
  ...
  validate({
    query: Joi.object().keys({
      ...
      long: Joi.number().min(-180).max(180).optional(), // Longitude should be between -180 and 180
      lat: Joi.number().min(-90).max(90).optional(), // Latitude should be between -90 and 90
    }),
  }),
  async function getAllVibesByPopularity(req, res) {
    try {
      const { customer } = req;
      let { page, limit, long, lat } = req.query;
      page = page - 1;
 ...
 *     description: Retrieve all active vibes for a specified merchant, sorted by their popularity (number of views). Includes optional localization of images based on the user's default language. The API supports pagination and returns metadata for better navigation.
 *     parameters:
 *       - in: query
 *         name: page
 *         schema:
 ...
 *           minimum: 1
 *           default: 10
 *         description: Number of vibes to retrieve per page.
 *     responses:
 *       '200':
