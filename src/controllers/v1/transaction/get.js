...
const CONTROLLER = [
  ...
  async function getAllCustomerTransactionV1Controller(req, res) {
    try {
      ...
      let final_transactions = Object.values(transactions_result);
      final_transactions = final_transactions.filter(obj => !(obj.network_points === 0 && obj.entity_points === 0));
      ...
      insertMessageLog(
        LOG_TYPE.ERROR,
        `Exception while fetching transactions list : ${error?.message}`,
        {
          message: error?.message,
          stack: error?.stack,
          errorObject: error,
        },
        "/v1/transaction/get-all",
        HTTP_VERBS.GET,
      ...
      return response.send(
        ...
        "Couldn't fetch transactions list",
        null,
        res,
        error
      );
 ...
 *                 message:
 *                   type: string
 *                   example: Could not fetch transactions
 */
